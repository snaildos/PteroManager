...
exports.run = async (client, message, args, guildConf, userConf) => {
    ...
    let option = args[0];
    if (!option) return client.sendEmbed(message.channel, "Missing argument", "User:\n\`\`\`view, buy, info\`\`\`\nAdmin:\n\`\`\`create, delete\`\`\`")
    if (!validOptions.includes(option.toLowerCase())) return client.sendEmbed(message.channel, "Invalid argument", "User:\n\`\`\`view, buy, info\`\`\`\nAdmin:\n\`\`\`create, delete\`\`\`")
    ...
    switch (option) {
                                                                                    ...
                                                                                    message.channel.awaitMessages(filter, {
                                                                                        ...
                                                                                        .then(collected => {
                                                                                            ...
                                                                                            message.channel.awaitMessages(filter, {
                                                                                                ...
                                                                                                .catch((e) => { return client.sendEmbed(message.channel, "You have not responded in time. Please start over."); });
        ...
        case "buy": {
            ...
            let params = {
                name: `${message.author.username}'s Server`,
                user: userConf.panelData.id,
                egg: product.eggID,
                nest: product.nestID,
                startup: null,
                docker_image: null,
            ...
            request.get(`${panel}/api/application/nests/${params.nest}/eggs/${params.egg}?include=variables`, {
                ...
                request.post(`${panel}/api/application/servers`, {
                    ...
                    await client.sendEmbed(message.channel, `Success!`, `[Click-Here](${guildConf.panelURL}/server/${identifier}) to view your server!`);

                    client.log("PTERODACTYL", `${guildConf.panelURL} -> created server!`);

                    client.serverDB.set(`${message.guild.id}.serversCreated`, guildConf.serversCreated++);
